...
// we will also clear our custom loop timers
	for(i=10;i<=60;i+=10){
		timer_clear(i);
	}
...
// the first parallel section of the code
//we will use timer value 10 to track the execution time of this loop
	timer_start(10);
#pragma omp parallel for default(shared) private(i,j,k) schedule(dynamic, 16)  
    for (k = 0; k < d[2]; k++) {
	for (j = 0; j < d[1]; j++) {
            for (i = 0; i < d[0]; i++) {
...
//second parallel section of the code
// we will use timer int 20 to track the execution time of this for loop
//starting the timer
	timer_start(20);
#pragma omp parallel for default(shared) private(i,j,k,ii,ii2,jj,ij2,kk) schedule(dynamic, 16)   
...
// we have defined our loop timers for int values 10,20,30,40,50,60
	printf("\nPrinting custom loop timers:\n");
	for(i=10;i<=60;i+=10){
		if(timer_read(i) != 0.0) {
			printf("Loop timer %2d: value is - %10.6f\n", i, timer_read(i));
		}
	}
}
...
//this is the third parallel section of the code
//we will use timer int 30 for this code block
	timer_start(30);
...
//this is the fourth parallel section of the code
//we will use timer int 40
	timer_start(40);
...
//this is the fifth parallel section of the code
//starting timer 50
	timer_start(50);
#pragma omp parallel default(shared) private(i,j,k,ii) shared(is)
{
...
//this is the sixth parallel section of the code
//we will use timer int 60
	timer_start(60);
...
#pragma omp barrier
//since all the threads have been serialized here, we can stop the timer inside the single block
#pragma omp single
  {    
	timer_stop(60);
    /* complex % real */
